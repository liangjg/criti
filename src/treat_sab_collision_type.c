                     ...
                     RNG_t *RNG)
{
    ...
    if(Get_loc_of_sab_el_erg(sab_nuc) != 0 && get_rand(RNG) * (SIG_sab_el + SIG_sab_inel) > SIG_sab_inel) {
        ...
        if(mode_el == 2) {
            if(get_rand(RNG) <= sab_k_el)
                LN = LN + N_el_mu;
            double ksi = (N_el_mu - 1) * get_rand(RNG) + 1;
            Loc = (int) (LN + ksi);
            exit_mu_lab = sab_nuc->XSS[Loc] + (sab_nuc->XSS[Loc - 1] - sab_nuc->XSS[Loc]) * (ksi - (int) (ksi));
        } else if(mode_el == 3) {
            Loc = LN + (int) (N_el_mu * get_rand(RNG));
            ...
            pi = sab_nuc->XSS[max] * get_rand(RNG);
        ...
        if(Get_sab_sec_erg_mode(sab_nuc) == 0)
            LE = (int) (get_rand(RNG) * Get_sab_inel_eout_num(sab_nuc));
        else {
            double rr = get_rand(RNG) * (Get_sab_inel_eout_num(sab_nuc) - 3);
            if(rr >= 1.0)
                LE = (int) (rr + 1);
            else {
                if(rr >= 0.5) {
        ...
        if(mode_inel == 2) {
            if(get_rand(RNG) <= sab_k_inel)
                LN = LN + LN_offset;
            double ksi = (N_inel_mu - 1) * get_rand(RNG) + 1;
            int Loc = (int) (LN + ksi);
            exit_mu_lab = sab_nuc->XSS[Loc] + (sab_nuc->XSS[Loc - 1] - sab_nuc->XSS[Loc]) * (ksi - (int) (ksi));
        } else if(mode_inel == 3) {
            int Loc = LN + (int) (N_inel_mu * get_rand(RNG));
