...
void treat_sab_colli_type(const nuclide_t *sab_nuc, particle_state_t *par_state, RNG_t *RNG_slave){
    ...
    double incident_erg = par_state->erg;
    ...
    if(Get_loc_of_sab_el_erg(sab_nuc) != 0 && get_rand_slave(RNG_slave) * (SIG_sab_el + SIG_sab_inel) > SIG_sab_inel){
        ...
        par_state->exit_erg = incident_erg;
        ...
        par_state->exit_erg = sab_nuc->XSS[LN] + sab_k_inel * (sab_nuc->XSS[LN + LN_offset] - sab_nuc->XSS[LN]);
        if(par_state->exit_erg <= 0){
            puts("exit energy in sab collision is out of range.");
            base_warnings++;
            par_state->exit_erg = EG0_CUTOFF;
        } else if(par_state->exit_erg <= 1.0E-11){
            if(par_state->exit_erg > EG0_CUTOFF)
                par_state->exit_erg = 1.0E-11;
        }
