                     ...
                     RNG_t *RNG)
{
    ...
    double exit_mu_lab;
    double incident_erg;
    double SIG_sab_el, SIG_sab_inel;
    nuc_xs_t *cur_nuc_xs;

    cur_nuc_xs = par_status->nuc_xs;
    incident_erg = par_status->erg;
    exit_mu_lab = ZERO;
    SIG_sab_el = cur_nuc_xs->el;
    SIG_sab_inel = cur_nuc_xs->inel;
    ...
    if(Get_loc_of_sab_el_erg(sab_nuc) != 0 && get_rand(RNG) * (SIG_sab_el + SIG_sab_inel) > SIG_sab_inel) {
        ...
        if(nMode_el == 2) {     ////// equally-probable angle bins.等概率余弦表
            if(get_rand(RNG) <= sab_k_el)
                LN = LN + N_el_mu;
            double ksi = (N_el_mu - ONE) * get_rand(RNG) + ONE;
            Loc = (int) (LN + ksi);
            ...
            double pi = sab_nuc->XSS[max] * get_rand(RNG);
