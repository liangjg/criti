...
void treat_sab_colli_type(const nuclide_t *sab_nuc, double SIG_sab_el, double SIG_sab_inel, double incident_erg,
    ...
    if(Get_loc_of_sab_el_erg(sab_nuc) != 0 && get_rand(&RNG_slave) * (SIG_sab_el + SIG_sab_inel) > SIG_sab_inel){
        *exit_erg_lab = incident_erg;
        ...
        if(nMode_el == 2){     ////// equally-probable angle bins.等概率余弦表
            if(get_rand(&RNG_slave) <= sab_k_el)
                LN = LN + N_el_mu;
            double ksi = (N_el_mu - 1) * get_rand(&RNG_slave) + 1;
            Loc = (int) (LN + ksi);
            ...
            Loc = LN + (int) (N_el_mu * get_rand(&RNG_slave));
            ...
            double pi = sab_nuc->XSS[max] * get_rand(&RNG_slave);
        ...
        if(Get_sab_sec_erg_mode(sab_nuc) == 0)
            LE = (int) (get_rand(&RNG_slave) * Get_sab_inel_eout_num(sab_nuc));
        else{
            double rr = get_rand(&RNG_slave) * (Get_sab_inel_eout_num(sab_nuc) - 3);
            if(rr >= 1.0)
                LE = (int) (rr + 1);
            else{
                if(rr >= 0.5){
        ...
        *exit_erg_lab = sab_nuc->XSS[LN] + sab_k_inel * (sab_nuc->XSS[LN + LN_offset] - sab_nuc->XSS[LN]);
        ...
        if(Mode_inel == 2){  // equally-probable angle bins.
            if(get_rand(&RNG_slave) <= sab_k_inel)
                LN = LN + LN_offset;
            double ksi = (N_inel_mu - 1) * get_rand(&RNG_slave) + 1;
            int Loc = (int) (LN + ksi);
            exit_mu_lab = sab_nuc->XSS[Loc] + (sab_nuc->XSS[Loc - 1] - sab_nuc->XSS[Loc]) * (ksi - (int) (ksi));
        } else if(Mode_inel == 3){ // qually-probable discrete angles.
            int Loc = LN + (int) (N_inel_mu * get_rand(&RNG_slave));
