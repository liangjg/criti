                        ...
                        double nu)
{
    ...
    for(int i = 0; i < fis_neu_num; i++){
        ...
        for(int j = 0; j < 3; j++){
            fixed_src_bank->pos[j] = par_status->pos[j];
            fixed_src_bank->dir[j] = par_status->exit_dir[j];
        }
        fixed_src_bank->erg = par_status->exit_erg;
        fixed_src_bank->wgt = par_status->wgt;
    }
}