                  ...
                  bool is_free_gas_col)
{
    ...
    get_ce_exist_erg_mu(nuc, RNG_slave, MT, par_status->erg_rel, &exit_erg_lab, &exit_mu_lab);

    /* 如果使用了自由气体模型则进一步转换出射方向和能量 */
    if(is_free_gas_col) {
        double atom_tmp, temp1, temp2;

        rotate_dir(exit_mu_lab, par_status->dir_vel, par_status->exit_dir, RNG_slave);
