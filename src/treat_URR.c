          ...
          double erg)
{
    ...
    double ratio = (erg - nuc->XSS[incident_erg_idx - 1]) / (nuc->XSS[incident_erg_idx] - nuc->XSS[incident_erg_idx - 1]);
    ...
    if(factor_flag != 0 || other_abs_flag == 0.) {
        ...
        if(factor_flag != 0) {
            cur_nuc_xs->el = cur_nuc_xs->el * el_factor;
            cur_nuc_xs->fis = cur_nuc_xs->fis * fis_factor;
            cur_nuc_xs->abs = cur_nuc_xs->abs * C_factor;
        }
    }
    ...
    cur_nuc_xs->abs = cur_nuc_xs->abs + abs_balance;
    if(cur_nuc_xs->abs < 0) {
        int NE = Get_erg_grid_num(nuc);
        cur_nuc_xs->abs = intplt_by_pos_fr(nuc->XSS, cur_nuc_xs->inter_pos + 2 * NE, cur_nuc_xs->inter_frac);
    }
