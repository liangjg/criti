...
int get_intplt_pos(const double *vec, const double key, int low, int high){
    int lo, hi, mi;
    lo = low;
...
int get_intplt_pos_fr(const double *vec, const double key, int low, int high, double *frac){
    int lo, hi, mi;
    lo = low;
    hi = high + 1;
    ...
    if(vec[lo - 1] == vec[lo]) *frac = ZERO;
    else *frac = (key - vec[lo - 1]) / (vec[lo] - vec[lo - 1]);
