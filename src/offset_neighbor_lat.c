...
/* -------------------------- private prototypes ---------------------------- */
static int
_offset_neighbor_lat_rect(universe_t *obj,
                          int lat_index,
                          int lat_bound_surf,
                          double pos[3]);

static int
_offset_neighbor_lat_hex(universe_t *obj,
                         int lat_index,
                         int lat_bound_surf,
                         double pos[3]);
                    ...
                    double pos[3])
{
    ...
    switch(obj->lattice_type) {
        case 1:filled_lat_univs_sz *= obj->scope[2];
            new_lat_index = _offset_neighbor_lat_rect(obj, lat_index, lat_bound_surf, pos);
            break;
        case 2:new_lat_index = _offset_neighbor_lat_hex(obj, lat_index, lat_bound_surf, pos);
            break;
    }
                          ...
                          double *pos)
{
    ...
    switch(lat_bound_surf) {
        case 1:new_index = lat_index - 1;
            pos[0] += obj->pitch[0];
            break;
        case 2:new_index = lat_index + 1;
            pos[0] -= obj->pitch[0];
            break;
        case 3:new_index = lat_index - obj->scope[0];
            pos[1] += obj->pitch[1];
            break;
        case 4:new_index = lat_index + obj->scope[0];
            pos[1] -= obj->pitch[1];
            break;
        case 5:new_index = lat_index - obj->scope[0] * obj->scope[1];
            pos[2] += obj->pitch[2];
            break;
        case 6:new_index = lat_index + obj->scope[0] * obj->scope[1];
            pos[2] -= obj->pitch[2];
            break;
    }
    return new_index;
