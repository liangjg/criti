                ...
                RNG_t *RNG)
{
    ...
    if(par_status->cell_tmp > 1.0E-24 && (par_status->erg < 400 * par_status->cell_tmp || nuc->atom_wgt <= 1.5)) {
        par_status->is_free_gas_col = true;
        if(cur_nuc_xs->ptable) return;
        treat_free_gas_model(par_status, RNG, nuc->atom_wgt);
        par_status->interp_N = get_intplt_pos_fr(nuc->XSS, par_status->erg_rel, 1, Get_erg_grid_num(nuc), &par_status->interp_K);
    } else {
