...
void find_neighbor_cell(particle_state_t *par_state){
    ...
    int bound_surf = par_state->bound_surf;
    ...
    if(univ->is_lattice){
        int lat_index = offset_neighbor_lat(univ, par_state->loc_cells[level], par_state->bound_surf, loc_pos);
        if(lat_index >= 0){
        ...
        int current_cell = par_state->loc_cells[level];
        ...
        if(found){
            par_state->loc_cells[level] = neighbor_cell->id;
